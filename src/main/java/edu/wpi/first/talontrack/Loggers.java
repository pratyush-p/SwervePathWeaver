...
final class Loggers {
   ...
   * ~/talontrack/talontrack.time.log
   */
  ...
  public static void setupLoggers() throws IOException {
    ...
    String directory = System.getProperty("user.home") + "/talontrack/logs";
    File folder = new File(directory);
    if (!folder.exists()) {
      folder.mkdir();
    }
    String time = DateTimeFormatter.ofPattern("YYYY-MM-dd-HH.mm.ss", Locale.getDefault()).format(LocalDateTime.now());
    final Handler fileHandler = new FileHandler(directory + "/talontrack." + time + ".log");
